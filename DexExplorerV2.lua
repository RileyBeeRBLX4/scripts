...
function checkTouchInGui(gui)
    if gui == nil then return false end
    local plrTouch = game:GetService("UserInputService")
    local guiPosition = gui.AbsolutePosition
    local guiSize = gui.AbsoluteSize
    ...
    if UserInputService.TouchEnabled then
        ...
        for _, touch in pairs(touches) do
            ...
            if touchPosition.X >= guiPosition.X and touchPosition.X <= guiPosition.X + guiSize.X and
               touchPosition.Y >= guiPosition.Y and touchPosition.Y <= guiPosition.Y + guiSize.Y then
                return true
            end
        end
    end
